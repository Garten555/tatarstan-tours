...
export async function POST(request: NextRequest) {
  try {
    ...
    const systemPrompt = [
      ...
      '- ВСЕГДА отвечай на вопросы о турах, используя информацию из списка доступных туров',
      '- Если пользователь спрашивает "какие есть туры", "покажи туры", "что есть" - ОБЯЗАТЕЛЬНО перечисли все доступные туры из списка',
      '- Если тура нет в списке - честно скажи об этом и предложи похожие варианты из списка',
      '- Если запрос непонятен - попробуй понять намерение пользователя и дай полезный ответ',
      ...
      '- На вопросы о турах ОБЯЗАТЕЛЬНО давай развернутые ответы с конкретными данными из списка',
      '- Если пользователь спрашивает про туры - ВСЕГДА отвечай, используя данные из списка',
      '- Если пользователь использует нецензурную лексику - вежливо попроси общаться культурно, но не будь слишком строгим',
      '- Если сообщение похоже на набор букв - попробуй понять намерение и дай полезный ответ',
      '',
      'Помни: ты должен быть полезным, точным, вежливым и ЕСТЕСТВЕННЫМ помощником! ВСЕГДА отвечай на вопросы о турах!',
    ].join('\n');
    ...
    // Но только если это действительно мат, а не случайное совпадение
    if (containsProfanity(aiText)) {
      // Проверяем, не является ли это ложным срабатыванием (например, слово "категория" содержит "еба")
      const lowerText = aiText.toLowerCase();
      const hasRealProfanity = PROFANITY_WORDS.some(profanity => {
      ...
      // Если это ложное срабатывание - оставляем ответ как есть
    }
